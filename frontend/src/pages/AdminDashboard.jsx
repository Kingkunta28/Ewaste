...
export default function AdminDashboard({
  ...
  const loadDashboardData = async () => {
    const [collectorData, userData, statData] = await Promise.all([
      api.listCollectors(),
      api.listUsers(),
      api.dashboardStats(),
    ]);
  ...
  return (
    <section className="role-panel role-admin">
      <div
        id="admin-reports"
        className="panel-hero admin-hero card admin-workspace-section"
      >
        <div>
          <p className="panel-kicker">Operations Center</p>
          <h2>Admin Dashboard</h2>
          <p>
            Monitor incoming pickups, assign collectors, and close completed
            requests quickly.
          </p>
        </div>
        <div className="report-controls">
          ...
          <button
            ...
            disabled={exportingReport || !reportMonth}
          >
      ...
      {stats ? (
        <div className="stats">
          ...
          <div className="stat">
            <strong>
              {users.filter((systemUser) => systemUser.is_active).length}
            </strong>
      ...
      <div
        id="admin-analytics"
        className="admin-analytics-grid admin-workspace-section"
      >
        <article className="card mini-chart-card">
          <div className="section-head">
            <h3>Requests by Month</h3>
            <p>Collection activity overview</p>
          </div>
          <div className="bar-chart" aria-label="Monthly requests chart">
            {[38, 55, 42, 72, 61, 88, 76, 92].map((height, index) => (
        ...
        <article className="card status-chart-card">
          <div className="section-head">
          ...
          <div
            ...
            style={{
              "--complete": `${requests.length ? Math.round((requests.filter((item) => item.status === "completed").length / requests.length) * 100) : 0}%`,
            }}
          >
            <span>
              {requests.length}
      ...
      <div
        id="admin-collectors"
        className="card table-shell admin-workspace-section"
      >
        <div className="section-head">
          <h3>Collectors</h3>
          <p>Collector accounts registered in the system.</p>
        </div>
        <div className="quick-actions collector-actions">
          <button
            className="action-primary"
            type="button"
            onClick={() => setShowCollectorRegistration(true)}
          >
          ...
          >
            ...
            {exportingReport ? "Generating PDF..." : "Generate PDF Report"}
          </button>
          <button
            className="action-secondary"
            type="button"
          ...
          <form className="form-grid" onSubmit={registerCollector}>
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  updateCollectorForm("first_name", e.target.value)
                }
                required
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  updateCollectorForm("last_name", e.target.value)
                }
                required
              />
            </label>
            ...
            <label>
              ...
              <div className="password-wrap">
                <input
                  ...
                  onChange={(e) =>
                    updateCollectorForm("password", e.target.value)
                  }
                  minLength={8}
                  required
                />
                <button
                  ...
                  aria-label={
                    showCollectorPassword ? "Hide password" : "Show password"
                  }
                  title={
                    showCollectorPassword ? "Hide password" : "Show password"
                  }
          ...
          <tbody>
            {collectors.length ? (
              collectors.map((collector) => (
                <tr key={collector.id}>
                  <td>
                    {`${collector.first_name || ""} ${collector.last_name || ""}`.trim() ||
                      "-"}
                  </td>
                  <td>{collector.username}</td>
                  <td>{collector.email || "-"}</td>
                  <td>{collector.profile__phone || "-"}</td>
                  <td>{collector.profile__address || "-"}</td>
                  <td>{collector.is_active ? "Active" : "Inactive"}</td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan={6}>No collectors registered.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      >
        <div className="section-head">
          <h3>System Users</h3>
          <p>Standard user accounts registered in the system.</p>
        </div>
        <table>
          <thead>
            <tr>
            ...
            {users.length ? (
              users.map((systemUser) => (
                <tr key={systemUser.id}>
                  ...
                  <td>{systemUser.profile__phone || "-"}</td>
                  <td>{systemUser.profile__address || "-"}</td>
                  <td>
                    {new Date(systemUser.date_joined).toLocaleDateString()}
                  </td>
                  <td>{systemUser.is_active ? "Active" : "Inactive"}</td>
                </tr>
              ))
            ) : (
              <tr>
              ...
              <tr key={req.id}>
                ...
                <td>
                  <select
                    ...
                    onChange={(e) =>
                      setCollectorSelection(req.id, e.target.value)
                    }
                  >
                ...
                <td>
                  <div className="table-actions">
                    ...
                    <button
                      ...
                      disabled={
                        !req.assigned_collector || req.status === "completed"
                      }
                    >
                      Complete
                    </button>

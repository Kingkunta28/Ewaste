...
import {
  ewasteBrandOptionsByItem,
  ewasteConditionOptions,
  ewasteItemOptions,
  initialRequestForm,
} from "../constants";
...
export default function UserDashboard({
  ...
  const selectedBrandOptions = ewasteBrandOptionsByItem[form.item_type] || [
    "Other",
  ];
    ...
    <section className="role-panel role-user">
      <div className="user-hero card">
        <div>
          ...
          <p className="user-subtitle">
            Submit your electronic waste pickup and track each request from
            pending to completion.
          </p>
        </div>
        ...
        <article className="card eco-impact-widget">
          <div className="widget-head">
            <div>
              ...
              <h3>Your positive footprint</h3>
            </div>
            <span className="widget-icon">♧</span>
          </div>
          <div className="impact-widget-stats">
            <div>
              <strong>{(completedCount * 3.4).toFixed(1)} kg</strong>
              <span>E-waste diverted</span>
            </div>
            <div>
              <strong>{(completedCount * 2.1).toFixed(1)} kg</strong>
          ...
          <div className="goal-row">
            <span>Monthly green goal</span>
            <strong>{Math.min(completedCount * 20, 100)}%</strong>
          </div>
          <div className="goal-bar">
            <i style={{ width: `${Math.min(completedCount * 20, 100)}%` }} />
          </div>
        </article>
        <article className="card recycling-tip">
          <span className="tip-art">♻</span>
          <span className="section-kicker">QUICK TIP</span>
          <h3>Protect your data first</h3>
          <p>
            Back up your files and sign out of all accounts before handing over
            a phone or computer.
          </p>
          <a href="#tips">Explore recycling tips →</a>
        </article>
      </div>

      <form
        id="new-pickup-request"
        className="card user-form-card"
        onSubmit={submit}
      >
        <div className="section-head">
          <h3>Request E-Waste Pickup</h3>
          <p>
            Fill in accurate item and location details for faster assignment.
          </p>
        </div>
        <div className="grid">
          <label>
      ...
      <div id="pickup-history" className="user-history">
        <div className="section-head">
          ...
          <p>
            Track all your submitted requests and their current processing
            status.
          </p>
        </div>
